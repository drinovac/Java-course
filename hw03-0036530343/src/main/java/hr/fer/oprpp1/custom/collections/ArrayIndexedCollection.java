...
public class ArrayIndexedCollection<T> implements List<T> {
	...
	private ArrayIndexedCollection(int size, T[] elements) {
		...
		this.size = Math.max(size, elements.length);
	 ...
	 * 
	 * @param o Object that is checked
	 ...
	 * 
	 * @param t Element that will be added
	 */
	@Override
	public void add(T t) {
		...
		try {
			...
			T[] new_arr = (T[]) new Object[this.elements.length * 2];
			for(int i = 0, h = this.elements.length; i < h; i++) {

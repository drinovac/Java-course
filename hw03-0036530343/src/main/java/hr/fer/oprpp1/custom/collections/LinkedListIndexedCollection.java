...
public class LinkedListIndexedCollection<T> implements List<T> {
	...
	public LinkedListIndexedCollection(Collection<T> collection) {
		if(collection instanceof ArrayIndexedCollection) {
			ArrayIndexedCollection<T> array = (ArrayIndexedCollection<T>) collection;
			for(int i = 0, h = collection.size(); i < h; i++) {
				this.add((T) array.toArray()[i]);
			}
		} else if(collection instanceof LinkedListIndexedCollection) {
			LinkedListIndexedCollection<T> array = (LinkedListIndexedCollection<T>) collection;
			for(ListNode left = array.first; left != null; left = left.next) {
				this.add((T) left.value);
			}
		}
	}
	...
	private static class ListNode<T> {
		...
		public ListNode(ListNode previous, ListNode next, T value) {
	...
	public void add(T t) {
		...
		if(this.first == null && this.last == null) {
			ListNode new_node = new ListNode(null, null, t);
			this.first = new_node;
			this.last = new_node;
			this.size = 1;
		} else {
			ListNode new_node = new ListNode(this.last, null, t);
	...
	public int indexOf(T t) {
		...
		for(ListNode left = this.first, right = this.last;
			...
			if(left.value.equals(t)) {
				return cnt;
			} else if(right.value.equals(t)) {
	...
	private static class ElementsGetterClass<T> implements  ElementsGetter<T> {
		 ...
		 */
		public ElementsGetterClass(LinkedListIndexedCollection<T> coll, long savedModificationCount ) {
	 ...
	 */
	@Override
	public ElementsGetterClass<T> createElementsGetter() {
